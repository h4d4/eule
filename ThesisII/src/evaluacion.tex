...
flujos implícitos. La incapacidad para detección de Flujos implícitos por parte
de FlowDroid, responde al tipo de análisis y las técnicas en que se fundamenta
la herramienta. Puesto que, 
% 
% \newline
...
implícitos.\newline
Con esto presente, se definen dos escenarios de análisis donde se incluyen u
omiten los testcases para flujos implícitos(ImplicitFlows\_ImplicitFlow 1, 2
y 4).
...
calcula la Precisión y Recall para cada uno de los excenarios.
...
mientras que el Prototipo presenta un porcentaje del  73\%.\newline
...
Los anteriores apartados del presente capítulo presentan un análisis detallado
para la evaluación entre FlowDroid vs Prototipo, y, JoDroid vs Prototipo. De los
resultados obtenidos se toman los correspondientes a la evaluación del conjunto
completo de testcases(conjunto de evaluación). La tabla\ref{tb:comparacion}
resume tales resultados.\newline Como muestran las tablas \ref{tb:resultados} y
\ref{tab:JoDroid-Prototipo}, el análisis de flujo de información basado en
lenguajes tipados de seguridad(en que se fundamenta la propuesta de análisis)
presenta un mejor desempeño, frente a las técnicas en que se basan FlowDroid y
JoDroid, análisis de flujo de datos mediante técnicas tainting y IFC(Information
Flow Control) mediante PDG y slicing, respectivamente.
