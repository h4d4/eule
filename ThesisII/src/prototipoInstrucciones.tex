...
En el directorio \small{\ttfamily{/home/estudiante/eule}} están los elementos
...
entrada para el genenrador de labels(LabelGenerator.jar), que devuleve la
versión jif de los mismos.

\textbf{Instrucciones de ejecución:}\newline
\textbf{(1)} Ejecutar el jar para la generación de los labels:
\begin{lstlisting}
...
\textbf{(2)} ejecutar el script setup.sh(basta con ejecutarlo una sóla vez)
\begin{lstlisting}
...
\begin{lstlisting}
estudiante@debianJessie:~/eule/FlowDroid$ java -jar FlowDroid.jar \
../DroidBench-master/apk/ImplicitFlows/ImplicitFlow4.apk
~/android-sdks/platforms/
\end{lstlisting}
